...
fn execute_value_op<'a, T: std::io::Write>(
	...
	Id => {
	    ...
	    match src {
		Value::Pointer(ref ptr) => { gc.increment(ptr.base); },
		_ => {}
	    }
	    match value_store.get(&dest) {
		Value::Pointer(ptr) => { gc.decrement(ptr.base); },
		_ => {}
	    }
	...
	Call => {
	    ...
	    let ret_val = execute(prog, callee_func, out, next_env, heap, gc, instruction_count)?.unwrap();
	    match ret_val {
		// If assigning to pointer, bump in gc
		Value::Pointer(ref pointer) => { gc.increment(pointer.base) },
		_ => {},
	    }
	    value_store.set(
		dest,
...
fn execute_effect_op<'a, T: std::io::Write>(
	...
	Return => {
		...
		Some(_) => {
			...
			Value::Pointer(ref ptr) => {
			    for garb in garbo.iter() {
				let pointer = Pointer { base: *garb, offset: 0 };
				if *garb != ptr.base { heap.free(&pointer)?; };
			    }
			},
			_ => {
			    for garb in garbo.iter() {
	...
	Call => {
		...
		match next_env.get(arg) {
		    Value::Pointer(ref ptr) => gc.increment(ptr.base),
		    _ => {}
		}
	    }

	    execute(prog, callee_func, out, next_env, heap, gc, instruction_count)?;
	}
	Store => {
	    let arg0 = get_arg::<&Pointer>(value_store, 0, args);
