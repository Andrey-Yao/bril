...
fn execute_value_op<'a, T: std::io::Write>(
  ...
  match *op {
	...
	if let Value::Pointer(ref ptr) = src {
            gc.increment(ptr.base);
	}
	if let Value::Pointer(ptr) = value_store.get(&dest) {
            if gc.decrement(ptr.base) {
		heap.free(ptr);
	    }
	}
    ...
    Alloc => {
      ...
      if let Value::Pointer(ref ptr) = res {
          gc.increment(ptr.base);
      }
	if let Value::Pointer(ptr) = value_store.get(&dest) {
	    if gc.decrement(ptr.base){
